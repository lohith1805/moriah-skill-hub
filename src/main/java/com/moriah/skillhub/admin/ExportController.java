 ...
 * Synchronous from the caller's point of view (the request blocks on the async generation, see
 * {@code ExportService}'s Javadoc) — there is no separate status/download endpoint in this
 * feature's scope. */
@RestController
...
public class ExportController {
    ...
    @Operation(summary = "Generate an XLSX export (users/revenue/audit) and return a presigned download URL")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Export generated, presigned URL returned"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Unsupported report type"),
    ...
    public ResponseEntity<ApiResponse<ExportResponse>> export(
            @PathVariable String report, @CurrentUserUuid String callerUuid) {

        return ResponseEntity.ok(ApiResponse.success(exportService.export(report, callerUuid)));
    }
}
